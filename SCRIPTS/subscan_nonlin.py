##########
# this is a script used for nonlinear GYRO scan,normally this may need to be avoid to use since it will take very very long time
...
base_loadOutputs={'out.gyro.run':OMFITgaCode,'out.gyro.profile':OMFITgaCode,'run_log':OMFITgaCode,\
                  'out.gyro.gbflux':OMFITgaCode,'out.gyro.gbflux_i':OMFITgaCode,'out.gyro.gbflux_mom':OMFITgaCode,'out.gyro.gbflux_n':OMFITgaCode,\
                  'out.gyro.k_perp_squared':OMFITgaCode,'out.gyro.kxkyspec':OMFITgaCode,\
                  'out.gyro.moment_zero':OMFITgaCode , 'out.gyro.moment_u':OMFITgaCode }
...
for para_val in para_rang:
    ...
    # set list
    inputs_node['input.gyro'][para_name]=para_val
    nPara=2
    while root['SETTINGS']['PHYSICS'].has_key('Para'+str(nPara)) and root['SETTINGS']['PHYSICS'].has_key('Range'+str(nPara)):
        inputs_node['input.gyro'][root['SETTINGS']['PHYSICS']['Para'+str(nPara)]]=root['SETTINGS']['PHYSICS']['Range'+str(nPara)][ncount]
        nPara=nPara+1
    inputs_node['input.gyro']['NONLINEAR_FLAG'] = 1
