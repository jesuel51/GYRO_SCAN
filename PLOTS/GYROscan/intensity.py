...
subplot(221)
count=0
for item in ipltspectrum:
    fn=root['OUTPUTScan'][para][str(item)]['nonlin']['out.tglf.potential_spectrum'].filename
    ll=readspectrum(fn)
...
count=0
for item in ipltspectrum:
    fn=root['OUTPUTScan'][para][str(item)]['nonlin']['out.tglf.density_spectrum'].filename
    ll=readspectrum(fn)
    semilogx(ll.T[0],ll.T[1],lab[count],linewidth=lw,markersize=ms,label=item)
    count=count+1
...
count=0
for item in ipltspectrum:
    fn=root['OUTPUTScan'][para][str(item)]['nonlin']['out.tglf.temperature_spectrum'].filename
    ll=readspectrum(fn)
    semilogx(ll.T[0],ll.T[1],lab[count],linewidth=lw,markersize=ms,label=item)
    count=count+1
...
count=0
for item in ipltspectrum:
    fn=root['OUTPUTScan'][para][str(item)]['nonlin']['out.tglf.nete_crossphase_spectrum'].filename
    ll=readspectrum(fn)
#    semilogx(ll.T[0],ll.T[1],lab[count],linewidth=lw,markersize=ms,label=item)
